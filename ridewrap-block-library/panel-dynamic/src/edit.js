	...
	return (
		<div { ...blockProps }>
			<SelectControl
				...
				options={ [
					{ label: '1/3 ( One-third)', value: 'one-third' },
					{ label: '2/3 ( Two-thirds)', value: 'two-third' },
					{ label: '1/2 ( One-half)', value: 'one-half' },
				] }
				onChange={( value ) => setAttributes({ panel_size: value })}
			/>
			<ToggleControl
			...
			<InnerBlocks
				...
				templateLock="all"
			/>
			<div class="editor-id-panel">
				<h5>Product ID</h5>
				<p>Add a product ID to enable below functionality.</p>
				<RichText
					className=''
					tagName="p"
					value={ attributes.id }
					onChange={ ( id ) => setAttributes( { id } ) }
					placeholder={ __( 'Product ID', 'gutenberg' ) }
				/>
			</div>
			<ToggleControl
